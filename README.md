...
- **DevContainer** - `Aspire-Full.DevContainer` project exposes `AddDevContainer()` so Aspire manages the Docker workspace + Python 3.15 free-threaded tooling
...
> The AppHost now builds the VS Code devcontainer image through `AddDevContainer()` (from `Aspire-Full.DevContainer`), so both Aspire and Docker Compose share the same relative context, Python 3.15 constraints, and named volumes.
```
