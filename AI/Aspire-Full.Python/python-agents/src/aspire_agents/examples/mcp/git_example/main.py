...
async def run(mcp_server: MCPServer, directory_path: str):
    # Check for local model configuration (e.g. vLLM, Ollama running on RTX GPU)
    model = None
    if os.getenv("LOCAL_MODEL_BASE_URL"):
        print(f"Using local model at {os.getenv('LOCAL_MODEL_BASE_URL')}")
        client = AsyncOpenAI(
            base_url=os.getenv("LOCAL_MODEL_BASE_URL"),
            api_key=os.getenv("LOCAL_MODEL_API_KEY", "sk-placeholder"),
        )
    ...
    agent = Agent(
        name="Assistant",
        instructions=(
            f"Answer questions about the git repository at {directory_path}, "
            "use that for repo_path"
        ),
