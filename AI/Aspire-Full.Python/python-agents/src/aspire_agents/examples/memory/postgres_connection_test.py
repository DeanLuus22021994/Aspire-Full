...
async def test_postgres_connection() -> None:
    ...
    # Use Any for creator to avoid static analysis issues when library is missing
...
if __name__ == "__main__":
    async# filepath: c:\Users\Dean\source\Aspire-Full - 1\AI\Aspire-Full.Python\python-agents\src\aspire_agents\examples\memory\postgres_connection_test.py
"""
Test PostgreSQL connection and session storage without requiring API keys.
...
if __name__ == "__main__":
    asyncio.run(test_postgres_connection()).run(test_postgres_connection())
