...
- Source of truth: `.config/python/lint/config.yaml` (YAML/JSON). Update this file to change the global line length or shared rules.
...
- Editors: VS Code and VS Code Insiders automatically pick up `.flake8` and `.pylintrc`. To prevent bundled extensions under `.vscode/` and `.vscode-test/` from triggering diagnostics, the workspace sets:
	...
	```
	The wrapper simply proxies to `run.py`, so both CLI and editor linting share the same filtering logic. Keep these settings in sync with `.config/python/lint/config.yaml` whenever new vendor directories need to be muted.
