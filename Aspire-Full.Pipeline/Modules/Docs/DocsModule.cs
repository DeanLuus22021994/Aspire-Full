...
public class DocsModule
{
    public Command GetCommand()
    {
        ...
        // Generate
        var generateCommand = new Command("generate", "Generate documentation");
        ...
        generateCommand.SetHandler(GenerateDocsAsync, apiOption, llmsOption, allOption);
        ...
        changelogCommand.SetHandler(GenerateChangelogAsync, versionOption);
        command.AddCommand(changelogCommand);

        return command;
    }

    private async Task GenerateDocsAsync(bool api, bool llms, bool all)
    {
        ...
        if (api || all)
        {
            ...
            await ProcessUtils.RunAsync("dotnet", ["build", "Aspire-Full.slnf", "--configuration", "Release"], root, silent: false);
            ...
            {
                // Check xmldoc2md
                var (code, _) = await ProcessUtils.RunAsync("dotnet", ["tool", "list", "-g"], silent: true);
                // This check is weak, better to try running it or check specific tool
                // But let's just try to install it if we fail? Or just install it always?
                // Better: check if command exists.
                // ProcessUtils doesn't have "which" equivalent easily cross-platform.
                // We'll just try to install it and ignore error if installed.
    ...
    private async Task GenerateChangelogAsync(string version)
    {
