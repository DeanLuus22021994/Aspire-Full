...
public class DockerComponent : IDiscoveryComponent
{
    public async Task<DiscoveryResult> DiscoverAsync(EnvironmentConfig config)
    {
        ...
        try
        {
            ...
            if (info.ExitCode == 0)
            {
                ...
                if (root.TryGetProperty("ServerVersion", out var sv))
                    details["Server Version"] = sv.GetString() ?? "?";
                if (root.TryGetProperty("KernelVersion", out var kv))
                    details["Kernel"] = kv.GetString() ?? "?";
                if (root.TryGetProperty("MemTotal", out var mt))
                    details["MemTotal"] = (mt.GetInt64() / 1024 / 1024) + " MB";
                if (root.TryGetProperty("NCPU", out var cpu))
                    details["CPUs"] = cpu.GetInt32().ToString();
                if (root.TryGetProperty("Containers", out var c))
                    details["Containers"] = c.GetInt32().ToString();
                if (root.TryGetProperty("ContainersRunning", out var cr))
                    details["Running"] = cr.GetInt32().ToString();
            }
        }
        ...
        if (compose.ExitCode == 0)
            details["Compose"] = compose.Output.Trim();
